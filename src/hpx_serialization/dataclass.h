...
public:
    ...
    Event() = delete;
#endif
    Event(string &name, size_t n_elem);
    Event(string const &name, vector<int> const &data);
    string print_state();
};
...
template <class Archive>
void Simple::serialize(Archive &ar, unsigned int version)
{
    ar &elem;
}

template <class Archive>
void Event::serialize(Archive &ar, unsigned int version)
{
    ar &name &data;
}
